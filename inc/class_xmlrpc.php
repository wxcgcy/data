        ...
        if ($handle) {
            ...
            if (strlen($buf)) {
                ...
                if (false !== $pos) {
                    ...
                    if (strlen($xml)) {
                        // 滤除非utf8字符，防止解码错误
                        $xml = str_replace(preg_replace('/([\x09\x0A\x0D\x20-\x7E]|[\xC2-\xDF][\x80-\xBF]|\xE0[\xA0-\xBF][\x80-\xBF]|[\xE1-\xEC\xEE\xEF][\x80-\xBF]{2}|\xED[\x80-\x9F][\x80-\xBF]|\xF0[\x90-\xBF][\x80-\xBF]{2}|[\xF1-\xF3][\x80-\xBF]{3}|\xF4[\x80-\x8F][\x80-\xBF]{2})/xs','',$xml),'',$xml);
                        $result = xmlrpc_decode($xml);
                    }
                    else {
                        $result = new xmlrpc_error(8, "empty data");
                    }
				}
				else {
